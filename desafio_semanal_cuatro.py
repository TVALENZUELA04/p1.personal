...
def seleccion_libros (tabla_ordenada):     
    libros_seleccionados = []
    for libro in tabla_ordenada:
        agregar = int(input(f"El libro {libro[1]}, desea agregarlo al carrito? (1 = si / 0 = no)"))
        if agregar == 1:
            libros_seleccionados.append(libro)
    return libros_seleccionados
...
carrito = seleccion_libros (tabla_ordenada)

suma_total = sum(map(lambda libro: libro[3], carrito))
...
print(f"El total de su compra es de: {suma_total}")